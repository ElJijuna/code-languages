...
4. Run `npm run check`, `npm run build`, and `npm run size`. If new languages push the `api (full catalog)` budget over its limit, raise it in `scripts/bundle-size-budget.mjs` in the same pull request.
